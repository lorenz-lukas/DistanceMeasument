...
def readCSV():
    d = []
    for i in xrange(4):
        data = pd.read_csv('txt/test3.txt', sep=";", header=None)
        d.append(data)
    return d
...
def sync(data):
    ...
    cap = cv2.VideoCapture('video/test3.MP4')# 1920x1080 and 3030FPS
        ...
        if ret:
            if frame_wait > 0:
                ...
                if(t[j] <= t[len(t)-1] and d[j] > 18 and start):
                    ...
                    cv2.imwrite('out-test3/3_{}.png'.format(t[j]),frame)
